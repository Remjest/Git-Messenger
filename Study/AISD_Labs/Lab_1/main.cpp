...
void rand_array(char arr[], int size) {
    ...
    /*
     * ПРИМЕЧАНИЕ: time(0) возвращает количество секунд, прошедших с 1 января 1970 года.
     * Так как в коде данная функция вызывается 4 раза подряд, между их вызовами проходит гораздо меньше времени,
     * чем 1 секунда. Поэтому srand в каждом вызове возвращает одну и ту же последовательность чисел, а все множества A,B,C,D
     * оказываются сгенерированными одинаково. Для решения данной прпоблемы была взята функция std::chrono::system_clock,
     * берущая более точное время.
     */
    ...
    // Перемешиваем массив letters
    for (int i = 0; i < n - 1; ++i) {
        int j = i + rand() % (n - i); // Выбираем случайный индекс из оставшейся части массива
        swap(letters[i], letters[j]); // Меняем местами текущий элемент с выбранным
    }

    // Копируем первые size элементов в массив arr
